...
user_diet::user_diet(nutrition_problem &p) : _portions(p.size()) {
...
std::vector<int> user_diet::disp(nutrition_problem &p, std::ofstream &fout) {
    ...
    //matplot::save(LOCAL_PATH_EXPORTS"results/macro-nutrients.svg");
    //matplot::save(LOCAL_PATH_EXPORTS"results/macro-nutrients-latex", "epslatex");
    ...
    matplot::ylabel("Nutritional Value");
    matplot::legend({"LB","Result","UB"});
    matplot::save(LOCAL_PATH_EXPORTS"results/nutrients.svg");
    matplot::save(LOCAL_PATH_EXPORTS"results/nutrients-latex", "epslatex");
    matplot::show();
...
void user_diet::preferences_inclusion_limit(nutrition_problem &p){
    switch (p.turn()) {
        ///Breakfast
    case 1: {
...
/// this function evaluates the optimization_model according to its category
double user_diet::evaluate(nutrition_problem &p) {
    ...
    //Checks the optimization_model to be evaluated
    /// static optimization_model refers to equations 3.1 and 3.2 on page 24
    /// static optimization_model with mealstime refers to equations 3.3 - 3.6 on page 26
    /// dinamic optimization_model without preferences refers to equations 3.7 - 3.9 on page 27
    /// dinamic optimization_model considering preferences refers to equations 3.7 - 3.9 on page 27
    ...
    //Static optimization_model of single meal
    ...
    // Mealstime models
        ...
        case 6: {
            ...
            break;
        throw std::logic_error("Please consider 6 mealstime and optimization_model options form 1 to 4");
        }
...
user_diet user_diet::crossover(nutrition_problem &p, user_diet &rhs) {
...
double user_diet::distance(nutrition_problem &p, user_diet &rhs,
                           double max_dist) {
    double hamming = 0.0;
    for (unsigned long i = 0; i < p.size(); ++i) {
